#!/usr/bin/env bash
# Build & push the JobSet π Estimator image, then deploy the per-namespace infra
# (controller + leader Service + Gateway + HTTPRoute + RBAC). Run setup_infra.sh first.
#
...
# GKE controller to reconcile them fresh (use if the Gateway is wedged).
RESET_GW=false
case "${1:-}" in
...
JOBSET_IMAGE="${REGISTRY}/jobset-pi:${IMAGE_TAG}"
...
gcloud artifacts repositories create "${ARTIFACT_REGISTRY_REPO}" \
  ...
  --description="JobSet pi estimator images" --project="${PROJECT_ID}" \
  || echo "Repo may already exist; continuing."

echo "=== Authenticating Docker to ${REGION}-docker.pkg.dev ==="
gcloud auth configure-docker "${REGION}-docker.pkg.dev" --quiet

echo "=== Building image (linux/amd64 for GKE nodes): ${JOBSET_IMAGE} ==="
# Context is the repo root so the image can include both app/ and frontend/.
docker build --platform linux/amd64 -t "${JOBSET_IMAGE}" -f "${ROOT}/app/Dockerfile" "${ROOT}"

echo "=== Pushing image ==="
docker push "${JOBSET_IMAGE}"
...
# is immutable) or on --reset-gateway.
...
if [ "${RESET_GW}" = true ] || { [ -n "${CUR_GW_CLASS}" ] && [ "${CUR_GW_CLASS}" != "${DESIRED_GW_CLASS}" ]; }; then
  ...
  kubectl -n "${NAMESPACE}" delete httproute jobset-route --ignore-not-found
  force_delete_gateway "${GATEWAY_NAME}"
fi

# Variables the infra manifests reference.
export NAMESPACE JOBSET_IMAGE
...
kubectl -n "${NAMESPACE}" rollout restart deployment/jobset-controller-deployment
kubectl -n "${NAMESPACE}" rollout status deployment/jobset-controller-deployment --timeout=600s || true
...
for i in {1..30}; do
  ...
  if [ -n "${GATEWAY_IP}" ]; then
    ...
    echo "  Demo UI/API:  http://${GATEWAY_IP}/"
    echo "  Health:       http://${GATEWAY_IP}/healthz"
    break
  fi
  sleep 10
done
